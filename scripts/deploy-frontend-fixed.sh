...
echo -e "${BLUE}ðŸš€ Deploying Frontend to Amplify${NC}\n"
...
if [ -z "$APP_ID" ] || [ "$APP_ID" = "None" ]; then
    echo -e "${RED}âŒ Error: Could not find Amplify App ID${NC}"
    echo -e "${YELLOW}ðŸ’¡ Deploy the frontend stack first:${NC}"
...
if [ ! -d "dist" ]; then
    echo -e "${RED}âŒ Error: Build directory 'dist' not found${NC}"
    exit 1
fi
...
echo -e "${GREEN}âœ“${NC} Package created (${ZIP_SIZE})"
echo ""
...
echo -e "${GREEN}âœ“${NC} Upload completed"
echo ""
...
if [ $? -eq 0 ]; then
    ...
    echo -e "${GREEN}âœ“${NC} Deployment initiated (Job ID: $JOB_ID)"
    echo ""
    ...
    while true; do
        ...
        case $STATUS in
            "SUCCEED")
                ...
                ;;
            "FAILED")
                echo -e "${RED}âŒ Deployment failed${NC}"
                exit 1
                ;;
            "CANCELLED")
                echo -e "${RED}âŒ Deployment was cancelled${NC}"
                exit 1
                ;;
            *)
                sleep 10
    ...
    echo -e "${RED}âŒ Amplify deployment failed${NC}"
